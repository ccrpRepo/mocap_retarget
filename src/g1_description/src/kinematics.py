...
class RobotPinocchio:
    def __init__(self, urdf_path, model_path=None, verbose=False, framesub = None):
        """
        初始化机器人模型。
    ...
    def compute_jacobian(self, q, frame_name):
        ...
        if self.model.existFrame(frame_name):
            ...
            return pin.getFrameJacobian(self.model, self.data, frame_id, pin.ReferenceFrame.LOCAL_WORLD_ALIGNED)
        else:
            print(f"Frame '{frame_name}' not found.")
            return None
    ...
    def head_inverse_kinematics(self, des_rotM):
        q_res = np.zeros(3)
        # calculate waist first
        root_pos = self.framesub.frames['root_pose'][0]['pos']
        root_rotM = self.framesub.frames['root_pose'][0]['rotM']
        head_pos = self.framesub.frames['lowerneck_pose'][0]['pos']
        head_rotM = self.framesub.frames['lowerneck_pose'][0]['rotM']
        
        head_Jac = self.compute_jacobian(self.q_cur, 'head_sphere')[3:6, 18:21]
